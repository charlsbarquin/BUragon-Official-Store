...
// Get bestsellers with sales count
$stmt = $pdo->query("SELECT p.*, COUNT(oi.id) as sales_count FROM products p LEFT JOIN order_items oi ON p.id = oi.product_id LEFT JOIN orders o ON oi.order_id = o.id AND o.status = 'completed' WHERE p.status = 'active' GROUP BY p.id ORDER BY sales_count DESC, p.created_at DESC LIMIT 12");
$products = $stmt->fetchAll();
?>
<main class="bestsellers-products">
    ...
    <div class="container">
        <div class="page-header animated-header">
            ...
            <p class="page-subtitle">Our most popular and highly-rated products loved by the BU community</p>
        </div>
        <?php if ($products): ?>
        ...
        <?php else: ?>
            <div class="empty-state">
                ...
                <p>We currently don't have any bestsellers listed. Please check back later.</p>
            </div>
        <?php endif; ?>
    </div>
</main>
